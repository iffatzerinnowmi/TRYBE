...
{
    public function index(StudyMatchingService $matching, FreeToPaidUnlockService $unlock)
    {
        $user = auth()->user();
         ...
         * FreeToPaidUnlockService is the single source of truth (also used
         * by the API and the applyToStudy gate), and only counts VOLUNTEER
         * studies — matching what this page actually displays.
         */
        $unlockProgress = $unlock->progress($user);
        $unlockTarget = $unlockProgress['target'];
        $completedCount = $unlockProgress['count'];
        $unlockRemaining = $unlockProgress['remaining'];
